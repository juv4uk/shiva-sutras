...
def annotate_sutra_base(sutra):
    ...
    if latent_triggers:
        ...
        has_shape = True # Treat latent sandhi as a shape trigger for domain setting
            
    # 5. Syntactic Replacement Pattern
    ...
    if (has_shape or has_syntactic_replacement or latent_triggers) and status == "no":
        status = "maybe"
...
def main():
    ...
    print(f"Annotation v5 Complete. Total: {len(annotated)}")
    ...
    audit_file = os.path.abspath(os.path.join(base_dir, '../../ksetra/astadhyayi/blind/stratified_audit_sample_v5.yaml'))
