...
def get_domain_and_prior(sutra_id):
    ...
    if (adhyaya, pada) in [("6", "2"), ("8", "1")]:
        ...
        prior = "low" # Not segmental phonology
    elif (adhyaya, pada) == ("1", "3"):
        domain = "grammatical_notation"
        prior = "low"
    else:
        domain = "segmental_phonology"
...
def annotate_sutra_base(sutra):
    ...
    # 1. Lexical Triggers (Exact word matches mostly, but substring is okay for text since sandhi happens)
    lexical_keywords = [
    ...
    # 3. Chapter Priors and Domains
    domain, prior = get_domain_and_prior(sutra['sutra_id'])
    
    # Domain override based on existing sutra type
    ...
    if domain in ["prosody_accent", "grammatical_notation"]:
        # If it's not segmental phonology, demote status but keep it alive
        if status == "yes":
            status = "maybe"
...
def main():
    ...
    for i in range(len(annotated)):
        ...
        if status == "yes":
            ...
            # If previous context was strong phonology and this sutra is very short
            if anuvrtti_active and words_count <= 3:
                ...
                # Long sutra without phonology resets the context
                anuvrtti_active = False
    ...
    print(f"Annotation v2 Complete. Total: {len(annotated)}")
    ...
    audit_file = os.path.abspath(os.path.join(base_dir, '../../ksetra/astadhyayi/blind/stratified_audit_sample_v2.yaml'))
