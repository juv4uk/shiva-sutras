...
def annotate_sutra_base(sutra):
    ...
    # 1. Lexical Triggers (Exact word matches mostly)
    lexical_keywords = [
    ...
    # 3. Derived Shape / Morpheme Triggers (Substring match allowed)
    shape_keywords = [
        "उपध", "आदि", "आदे", "अन्त", "अल्", "एकाल्", "एकाच", "एकाच्", "अनेकाच", "अनेकाच्", "द्व्यच", "द्व्यच्",
        "गुरु", "लघु", "आत्", "अतः", "इतः", "उतः", "ऋतः", "वर्ण", "अनुदात्त", "उदात्त"
    ]
    has_shape = False
    ...
    # 4. Syntactic Replacement Pattern (e.g. 6/1 followed by 1/1)
    # This catches "Target(Genitive) -> Replacement(Nominative)" patterns like 'एः ६/१ उः १/१'
    has_syntactic_replacement = False
    if re.search(r'\S+\s+६/[१२३]\s+\S+\s+१/[१२३]', padaccheda):
    ...
    if (has_shape or has_syntactic_replacement) and status == "no":
        status = "maybe"
...
def main():
    ...
    print(f"Annotation v4 Complete. Total: {len(annotated)}")
    ...
    audit_file = os.path.abspath(os.path.join(base_dir, '../../ksetra/astadhyayi/blind/stratified_audit_sample_v4.yaml'))
